...
import { deleteStopAction } from "../actions/deleteStopAction";
...
export function StopDeleteDialog({ stopId, stopName, onSuccess, onCancel }: StopDeleteDialogProps) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");

  async function handleDelete() {
    setLoading(true);
    const result = await deleteStopAction(stopId);
    if (result.success) {
      onSuccess();
    } else {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="text-center">
        <h3 className="text-lg font-bold text-gray-800">¿Eliminar parada?</h3>
        <p className="text-sm text-gray-500 mt-1">
          Estás por eliminar <span className="font-semibold text-gray-700">"{stopName}"</span>. Esta acción no se puede deshacer.
        </p>
      </div>
